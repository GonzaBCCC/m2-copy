...
Iinp =G11_Laplace_Equation_Axb(I, mask, param);
...
Iinp(:,:,1)=G11_Laplace_Equation_Axb(I(:,:,1), mask(:,:,1), param);
Iinp(:,:,2)=G11_Laplace_Equation_Axb(I(:,:,2), mask(:,:,2), param);
Iinp(:,:,3)=G11_Laplace_Equation_Axb(I(:,:,3), mask(:,:,3), param);
...
mask = I_ch1==1 & I_ch2==0 & I_ch3==0;  % compute mask looking for red pixels (255, 0, 0)
imwrite(mask, 'Image_to_Restore_mask.png');  % save mask image to disk
% Show computed mask applied to image, for demo purposes
% I_masked = I.*repmat(mask,[1,1,3]);
% figure(1);
% subplot(1,2,1); imshow(I); title('Original Image');
% subplot(1,2,2); imshow(I_masked); title('Masked image (red = lost info)');
% -------------------------------------------------------------------------
...
imshow(I);
title('Before')
Iinp(:,:,1)=G11_Laplace_Equation_Axb(I_ch1, mask, param);
Iinp(:,:,2)=G11_Laplace_Equation_Axb(I_ch2, mask, param);
Iinp(:,:,3)=G11_Laplace_Equation_Axb(I_ch3, mask, param);
