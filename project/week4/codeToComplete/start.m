...
file2read = ['TestSet/GoalImage_w1.png'];
I0 = imread(file2read);
% I0 = imresize(I0,0.25); %comment if you don't want to downscale the images
I0 = (double(I0)/255);  % image to fill

%load mask
%TODO: Save on the testSet Folder the mask of the image to resotre.
file2read = ['TestSet/mask10.png'];
mask = imread(file2read);
% mask = imresize(mask,0.25); %%comment if you don't want to downscale the images
...
for j=1:Nresults
    ...
    completed_imgs{j,1} = G11_fill_image(I0,similar_images{j,1},mask,mask_extended);
