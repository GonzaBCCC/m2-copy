...
numPatches = 2; %TODO: Number of states or clusters in our case
% end TODO 1
...
edgeStruct=G11_CreateGridUGMModel(height, width, numPatches );
% end TODO 2:
...
distances = bwdist(mask); %verificar si no es mask_extended.
for p=1:numNodes %TODO: For each row in Vp (each row is a node) assign potentials
    ...
    if mask_im(i,j) == 1 %if i,j belongs to the mask, then it has probabilty 0 of being from the canditate image and 1 of being mask
        Vp(p,:) = [0 1]; 
    else %if i,j DOES NOT belong to the mask, then it has probabilty 1 of being from the canditate image and a probability depending of the distance of being mask
        Vp(p,:) = [1 exp(-(k*distances(i,j))^3)];
    end
end
% END TODO 3:
...
SSD= sum((im_hole-im_complet).^2,3);
...
for e = 1:edgeStruct.nEdges
    ...
    %potential = norm(gradient(SSD(i1,j1))
